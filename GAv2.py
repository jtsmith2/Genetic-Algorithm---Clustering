...
parameters = 11			#Number of variables in X (not including constant term)
...
def main_program(): #Main program run by last line of this code.
	...
	dataX = genfromtxt('salesX.csv', delimiter=",")
	dataY = genfromtxt('salesY.csv', delimiter=",")
...
def objfunc(str): #Returns the value of the objective function for the given string
	...
	clusterdataY = []
	index = 0
	for count in incluster:
		clusterdataX[:0] = zeros((count, parameters))
		clusterdataY[:0] = zeros((count))
	
	#Sorts the data into the correct cluster data array
	clusterinsertcount = zeros((clusters))
	for i in xrange(strlength):
		print clusterdataX[int(str[i])][clusterinsertcount[int(str[i])]] 
		print dataX[i]
	...
	for i in xrange(clusters):
		ssr[i] = ols(clusterdataY[i], clusterdataX[i], 'y', 'x'+range(parameters)).sse
